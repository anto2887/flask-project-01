...
def compare_and_update(week, league):
    ...
    try:
        ...
        if scraped_data_list is None:
            current_app.logger.info("No scraped data was found. Exiting the function early.")
            return

        current_app.logger.info("Scraped data list: %s", scraped_data_list)
        ...
        if not posts:
            current_app.logger.info("No posts found. Exiting the function early.")
            return

        for post in posts:
            user_input_results = score_input(post.body)
            current_app.logger.info("User input results for post ID %s: %s", post.id, user_input_results)
            ...
            for result in user_input_results:
                ...
                if not actual_result:
                    current_app.logger.info(f"No actual result found for match: {team1} vs {team2}")
                    continue
                ...
                current_app.logger.info(f"Updating points for author_id {post.author_id}, week {week}, season {season}. New total: {user_result.points}")

                try:
                    db.session.commit()
                    db.session.refresh(user_result)
                    db.session.refresh(user_prediction)
                    current_app.logger.info(f"Updated UserResults: author_id {user_result.author_id}, season {user_result.season}, total points: {user_result.points}")
                except Exception as e:
                    db.session.rollback()
                    current_app.logger.error("Error during commit: %s", str(e))

    except Exception as e:
        current_app.logger.exception("An error occurred during the compare_and_update process.")
        raise e